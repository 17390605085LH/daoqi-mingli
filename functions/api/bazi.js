...
function getLunarYear(solarYear, solarMonth, solarDay) {
  const lc = LICHUN_TABLE[solarYear] || [2, 4];
  const lcMonth = lc[0], lcDay = lc[1];
  // 立春前出生 → 属前一年
  if (solarMonth < lcMonth || (solarMonth === lcMonth && solarDay < lcDay)) {
    return solarYear - 1;
  }
  return solarYear;
}
...
function calcYearPillar(year, month, day) {
  ...
  const idx = (lunarYear - 4 + 60) % 60;
  return [STEMS[idx % 10], BRANCHES[idx % 12]];
}

// 简单农历→阳历近似转换（仅用于月日调整）
// 农历年首通常在阳历1月下旬~2月下旬
function lunarToSolarApprox(lunarYear, lunarMonth, lunarDay) {
  // 查找该农历年对应的阳历春节日期
  // 使用简化表：1940-2030年春节阳历日期
  const CHUNJIE = {
  ...
  const cj = CHUNJIE[lunarYear] || [2, 5];
  // 春节 = 农历正月初一
  // 累加天数：每月按29.5天近似
  let daysFromCJ = Math.round((lunarMonth - 1) * 29.5 + (lunarDay - 1));
  
  // 从春节日期累加
  const cjDate = new Date(lunarYear, cj[0] - 1, cj[1]);
  const solarDate = new Date(cjDate.getTime() + daysFromCJ * 86400000);
...
function calcMonthPillar(yearStem, month, day) {
  ...
  let adjMonth = month;
  const jieDay = (JIEQI_BOUNDARY[month] || {}).day || 5;
  if (day < jieDay) {
    adjMonth = month - 1;
...
function getMingGong(monthBranch, hourBranch) {
  // 寅为正月(0), 卯为二月(1), 辰为三月(2)...
  // 顺数生月：从寅(index=2)开始
  const monthIdx = BRANCHES.indexOf(monthBranch);
  // 寅=正月=0, 卯=二月=1, ..., 丑=十二月=11
  const monthNum = (monthIdx - 2 + 12) % 12 + 1; // 转换为月份数字 1-12
  
  // 从寅宫起正月，顺数至生月
  const tempIdx = (2 + monthNum - 1) % 12;
  
  // 从该宫起子时，逆数至生时
  const hourIdx = BRANCHES.indexOf(hourBranch);
  const mingIdx = (tempIdx - hourIdx + 12) % 12;
...
function getShenGong(monthBranch, hourBranch) {
  const monthIdx = BRANCHES.indexOf(monthBranch);
  const monthNum = (monthIdx - 2 + 12) % 12 + 1;
  const tempIdx = (2 + monthNum - 1) % 12;
  const hourIdx = BRANCHES.indexOf(hourBranch);
  const shenIdx = (tempIdx + hourIdx) % 12;
  return shenIdx;
}
...
function calcZiweiV2(yearStem, yearBranch, monthBranch, dayStem, dayBranch, hourBranch, gender) {
  ...
  // 1. diao值（紫微星所在，倪海厦一六分隔法）
  const diao = getDiao(dayStem, dayBranch);

  // 2. 命宫（陈希夷标准法：月支顺数+时支逆数）
  const mingGong = getMingGong(monthBranch, hourBranch);

  // 3. 身宫（陈希夷标准法：月支顺数+时支顺数）
  const shenGong = getShenGong(monthBranch, hourBranch);
...
export async function onRequestGet(context) {
  try {
    ...
    let year = parseInt(params.get('year')) || 1990;
    ...
    // === 农历→阳历转换 ===
    if (calendar === 'lunar') {
      const solar = lunarToSolarApprox(year, month, day);
      year = solar.year;
      month = solar.month;
      day = solar.day;
    }
    ...
    // 四柱计算
    const [yearGan, yearZhi] = calcYearPillar(year, month, day);
    ...
    // 用神
    const yongShenRaw = calcYongShen(dayGan, strength, geJu, tiaoHou, wuxingCount);

    // 性格
    const personality = analyzePersonality(dayGan, strength, geJu);

    // 幸运指南
    const luckyGuide = calcLuckyGuide(yongShenRaw);

    // 流年
    const now = new Date();
    const currentYear = now.getFullYear();
    const [cyGan, cyZhi] = calcYearPillar(currentYear, now.getMonth()+1, now.getDate());
    const liuNian = analyzeLiuNian(cyGan, cyZhi, pillars, yongShenRaw, dayGan);
    ...
    // 紫微斗数V2
    const ziweiRaw = calcZiweiV2(yearGan, yearZhi, monthZhi, dayGan, dayZhi, hourZhi, gender);

    // === 紫微斗数输出规范化 ===
    // ziweiRaw.mingGong = { palace: "命宫", branch: "寅" }
    const mingPalaceName = ziweiRaw.mingGong.palace;
    const shenPalaceName = ziweiRaw.shenGong.palace;
    const pd = ziweiRaw.palacesData;

    // 构建palaceDetails（含siHuas数组）
    const palaceDetails = {};
    for (const pName of ZW_PALACES) {
      const d = pd[pName] || {};
      const mainStars = d.mainStars || [];
      const mainStarBrightness = {};
      mainStars.forEach(s => { mainStarBrightness[s] = d[s] || '平'; });

      // 四化数组
      const siHuas = (d.hua || []).map(h => {
        const match = h.match(/^(.+)化(.+)$/);
        return match ? { type: match[2] === '禄' ? 'lu' : match[2] === '权' ? 'quan' : match[2] === '科' ? 'ke' : 'ji', star: match[1] } : { type: 'ke', star: h };
      });

      palaceDetails[pName] = {
        mainStars,
        mainStarBrightness,
        luckyStars: d.luckyStars || [],
        fierceStars: d.fierceStars || [],
        auxStars: d.auxStars || [],
        siHuas
      };
    }
    ...
    const ziwei = {
      ...
      shenPalace: shenPalaceName,
      mingStars: palaceDetails[mingPalaceName].mainStars,
      shenStars: palaceDetails[shenPalaceName].mainStars,
      wuxingJu: ziweiRaw.wuxingJu,
      ziweiPosition: ziweiRaw.ziweiPalace,
      mingInterpret: ziweiRaw.mingInterpret,
      shenInterpret: ziweiRaw.shenInterpret,
      palaceDetails,
      keyAnalysis: ziweiRaw.keyAnalysis,
      diao: ziweiRaw.diao
    };
    ...
    const result = {
      ...
      yongShen: {
        shen: yongShenRaw.yongShen,
        yongShen: yongShenRaw.yongShen.join('、'),
        yongWuxing: yongShenRaw.yongShen,
        avoid: yongShenRaw.avoid,
        principle: yongShenRaw.principle
      },
      dayMaster: dayGan,
